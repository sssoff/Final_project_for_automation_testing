...
def test_autoriz_valid_email_pass(selenium):
    """EXP-001 Тест авторизации с правильными (валидными) значениями e-mail и паролем."""
...
def test_autoriz_invalid_email_pass(selenium, incor_email, incor_passw):
    """EXP-002,003 "Проверка входа пользователя с невалидным email и паролем:
...
def test_menu_type_autoriz(selenium):
    """EXP-005 Проверка названия таб в меню выбора типа авторизации."""
    try:
        page = AuthPage(selenium)
        ...
        print('Ошибка в имени таб Меню типа аутентификации')


def test_menu_of_type_active_autoriz(selenium):
    """EXP-006 Проверка выбора таб по умолчанию в Меню выбора типа авторизации."""
    page = AuthPage(selenium)
